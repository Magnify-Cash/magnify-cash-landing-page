...
const Navigation = () => {
  ...
  return (
    <>
            ...
            {navigationItems.map((item) => (
              item.external ? (
                ...
                <button
                  ...
                  className={`text-lg transition-colors relative group cursor-pointer hover-lift ${
                    activeSection === item.path ? 'text-primary font-medium' : 'text-secondary hover:text-primary'
                  }`}
                >
                  {item.label}
                  <span className={`absolute -bottom-1 left-0 h-0.5 bg-gradient-to-r from-blue-600 to-pink-600 transition-all ${
            ...
            {navigationItems.map((item) => (
              item.external ? (
                ...
                <button
                  key={item.label}
                  onClick={() => scrollToSection(item.path)}
                  className={`block w-full text-left py-2 transition-colors hover-lift ${
                    activeSection === item.path ? 'text-primary font-medium' : 'text-secondary hover:text-primary'
                  }`}
                >
                  {item.label}
      ...
      <button
        onClick={scrollToTop}
        className={`fixed bottom-6 right-6 p-3 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-full shadow-lg transition-all duration-300 hover:scale-110 z-50 ${
