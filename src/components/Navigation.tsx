      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-background/80 backdrop-blur-xl shadow-sm' : 'bg-transparent'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-3 hover:opacity-80 transition-opacity hover-lift">
            <img alt="Magnify Cash Logo" className="h-8 w-8 sm:h-10 sm:w-10 rounded-[20%]" src="/lovable-uploads/7c5aac4e-c1ff-484f-a5c5-4b1cfc57a4bb.png" />
            <span className="text-xl sm:text-2xl font-semibold">Magnify Cash</span>
          </Link>
          
          <div className="hidden md:flex items-center space-x-6 lg:space-x-10">
            {navigationItems.map(item => item.external ? <a key={item.label} href={item.path} target="_blank" rel="noopener noreferrer" className="text-secondary hover:text-primary transition-colors relative group cursor-pointer text-lg hover-lift">
                  {item.label}
                ...
                </a> : <button key={item.label} onClick={() => scrollToSection(item.path)} className={`text-lg transition-colors relative group cursor-pointer hover-lift ${activeSection === item.path ? 'text-primary font-medium' : 'text-secondary hover:text-primary'}`}>
                  {item.label}
          ...
          <div className="flex items-center space-x-4">
            <a href="https://worldcoin.org/ecosystem/app_cfd0a40d70419e3675be53a0aa9b7e10" target="_blank" rel="noopener noreferrer" className="hidden sm:block">
        ...
        top: '5rem'
          ...
          <div className="px-4 py-6 space-y-4">
            {navigationItems.map(item => item.external ? <a key={item.label} href={item.path} target="_blank" rel="noopener noreferrer" className="block py-2 text-secondary hover:text-primary transition-colors hover-lift">
                  {item.label}
                </a> : <button key={item.label} onClick={() => scrollToSection(item.path)} className={`block w-full text-left py-2 transition-colors hover-lift ${activeSection === item.path ? 'text-primary font-medium' : 'text-secondary hover:text-primary'}`}>
                  {item.label}
                </button>)}
            <a href="https://worldcoin.org/ecosystem/app_cfd0a40d70419e3675be53a0aa9b7e10" target="_blank" rel="noopener noreferrer" className="block pt-4">
